  ...
  try {
    ...
    if (!result.ok) {
      if (result.error === "Status not completed") {
        return res
          .status(200)
          .json({ message: "Webhook ignored: status not completed" });
      }
    ...
    if (
      ...
      try {
        ...
        console.error(
          "Failed to fetch Bolna execution:",
          fetchError && fetchError.stack ? fetchError.stack : fetchError,
        );
        return res
          .status(502)
          .json({ error: "Failed to retrieve execution details from Bolna" });
      }
    }
